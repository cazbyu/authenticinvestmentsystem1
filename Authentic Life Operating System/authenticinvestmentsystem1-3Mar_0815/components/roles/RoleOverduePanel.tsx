...
import {
  ...
  RoleOverdueResult,
  RoleOverdueTask,
} from '@/lib/roleOverdue';
import { eventBus, EVENTS } from '@/lib/eventBus';
import { Task, TaskCard } from '@/components/tasks/TaskCard';

/**
 * RoleOverduePanel — R-3 sibling of components/wellness/ZoneOverduePanel.tsx.
 *
 * Renders past-due pending/in_progress tasks for the role-side
 * MY SPACE > Overdue tile.
 *
 * Owns its own data fetching (calls fetchRoleOverdue from R-1, caches
 ...
 * live in roles.tsx — same pattern as RoleUpcomingPanel.
 *
 * Single source: tasks (no events — overdue is a task-only concept;
 * see lib/roleOverdue.ts header for rationale, mirroring zoneOverdue.ts).
 */

export interface RoleOverduePanelProps {
  roleId: string;
  userId: string;
...
function taskRowToTask(row: RoleOverdueTask): Task {
  return {
    ...row,
    title: row.title ?? '',
    type: row.type ?? 'task',
...
export function RoleOverduePanel({
  roleId,
  userId,
  onTaskComplete,
  onTaskDelete,
  onTaskPress,
}: RoleOverduePanelProps) {
  ...
  const load = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        const result: RoleOverdueResult = await fetchRoleOverdue(
        ...
        if (signal?.aborted) return;
        console.error('[RoleOverduePanel] load failed', e);
