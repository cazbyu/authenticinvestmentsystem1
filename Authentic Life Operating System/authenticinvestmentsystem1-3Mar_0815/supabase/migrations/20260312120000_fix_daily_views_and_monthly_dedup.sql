-- ============================================================
-- Fix v_daily_goal_actions: Count completed tasks by completed_at,
-- not by note existence
-- ============================================================
...
SELECT
  ...
  gj.goal_type,
  (t.completed_at AT TIME ZONE COALESCE(u.timezone, 'UTC'))::date AS completion_date,
...
GROUP BY
  ...
  gj.goal_type,
  (t.completed_at AT TIME ZONE COALESCE(u.timezone, 'UTC'))::date,
...
-- ============================================================
-- Fix v_daily_role_investments: Count completed tasks and active
-- deposit ideas by completed_at/created_at, not by notes
-- ============================================================
...
WITH task_activities AS (
  SELECT
    t.user_id,
    rj.role_id,
    (t.completed_at AT TIME ZONE COALESCE(u.timezone, 'UTC'))::date AS activity_date,
  ...
  WHERE t.deleted_at IS NULL
    AND t.status = 'completed'
    AND t.completed_at IS NOT NULL
  GROUP BY t.user_id, rj.role_id, (t.completed_at AT TIME ZONE COALESCE(u.timezone, 'UTC'))::date
),
...
-- ============================================================
-- Fix v_daily_domain_balance: Count completed tasks by
-- completed_at, not by notes
-- ============================================================
...
WHERE t.deleted_at IS NULL
  AND t.status = 'completed'
  AND t.completed_at IS NOT NULL
GROUP BY t.user_id, dj.domain_id, (t.completed_at AT TIME ZONE COALESCE(u.timezone, 'UTC'))::date, d.name;

-- ============================================================
-- Fix get_month_dates_with_items: Deduplicate recurring tasks
-- and events per day
-- ============================================================
...
BEGIN
  ...
  -- Deduplicated tasks: one per title per day
  deduped_tasks AS (
    SELECT DISTINCT ON (
    ...
    ORDER BY (t.completed_at AT TIME ZONE v_user_timezone)::date, t.title, t.completed_at DESC
  ),
  ...
  -- Deduplicated events: one per title per day
  deduped_events AS (
    SELECT DISTINCT ON (
