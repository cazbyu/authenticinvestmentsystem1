/**
 * useGoogleCalendarSync Hook (Web version)
 *
 * Provides incremental Google Calendar sync with:
 * - Background sync every 10 minutes
 * - Sync on tab focus (replaces React Native AppState)
...
export function useGoogleCalendarSync(isCalendarTabActive: boolean = false) {
  ...
  const refreshTokenIfNeeded = useCallback(async (connection: GoogleCalendarConnection): Promise<string | null> => {
    // PRIORITY 1: Fresh provider_token from current Supabase session
    try {
    ...
    // PRIORITY 2: Refresh Supabase session for new provider_token
    try {
    ...
    // PRIORITY 3: Check if stored token is still valid (5+ minutes remaining)
    ...
    // PRIORITY 4: Netlify function with refresh_token
    ...
    // FALLBACK: Try existing stored token
  ...
  // Start/stop background sync based on page visibility (web equivalent of AppState)
  useEffect(() => {
    ...
    // Start sync when hook mounts (page is visible)
