...
// ============================================================================
// EMBEDDED HELPER: Planned Actions for Week (no external file needed)
// ============================================================================
...
interface PlannedActionsResult {
  leadingIndicators: {
    count: number;
    totalTarget: number;
    totalActual: number;
...
async function fetchPlannedActionsForWeek(): Promise<PlannedActionsResult> {
  const emptyResult: PlannedActionsResult = {
    leadingIndicators: { count: 0, totalTarget: 0, totalActual: 0 },
    boostActions: { count: 0, completed: 0, pending: 0 },
    week: null,
    timeline: null,
  };
  ...
  try {
    ...
    // Get active timeline (global first, then custom)
    let timeline: { id: string; source: 'global' | 'custom'; title?: string } | null = null;

    const { data: globalTimeline } = await supabase
      .from('0008-ap-user-global-timelines')
      .select('id, title, global_cycle:0008-ap-global-cycles(title, cycle_label)')
      .eq('user_id', user.id)
      .eq('status', 'active')
    ...
    if (globalTimeline) {
      timeline = {
        ...
        source: 'global',
        title: globalTimeline.title || (globalTimeline.global_cycle as any)?.title,
      };
    } else {
      const { data: customTimeline } = await supabase
      ...
      if (customTimeline) {
        timeline = { id: customTimeline.id, source: 'custom', title: customTimeline.title };
      }
    }
    ...
    if (!weekData) {
      console.log('[fetchPlannedActionsForWeek] No current week found');
    ...
    const { data: weekPlans } = await supabase
      ...
      .select(`
        task_id,
        target_days,
        task:0008-ap-tasks!inner(id, title, user_id, status, deleted_at)
      `)
    ...
    if (taskIds.length > 0) {
      const { data: completedOccurrences } = await supabase
        ...
        .eq('status', 'completed')
        .gte('due_date', currentWeek.startDate)
        .lte('due_date', currentWeek.endDate)
        .is('deleted_at', null);

      totalActual = (completedOccurrences || []).length;
    }

    const totalTarget = validPlans.reduce((sum: number, wp: any) => sum + (wp.target_days || 0), 0);
    ...
    if (timeline.source === 'global') {
      const { data } = await supabase
        ...
        .select(`
          ...
          goal_join:0008-ap-universal-goals-join!inner(
            twelve_wk_goal_id,
            goal:0008-ap-goals-12wk!inner(id, title, user_global_timeline_id)
          )
        `)
        .eq('user_id', user.id)
      ...
      const { data } = await supabase
        ...
        .select(`
          ...
          goal_join:0008-ap-universal-goals-join!inner(
            custom_goal_id,
            goal:0008-ap-goals-custom!inner(id, title, custom_timeline_id)
          )
        `)
        .eq('user_id', user.id)
        .is('recurrence_rule', null)
    ...
    const boostCompleted = boostTasks.filter((t: any) => t.status === 'completed').length;
    ...
    return {
      ...
      boostActions: {
        count: boostTasks.length,
        completed: boostCompleted,
        pending: boostTasks.length - boostCompleted,
      },
      week: currentWeek,
      timeline,
    };
    ...
    console.error('[fetchPlannedActionsForWeek] Error:', error);
    return emptyResult;
  }
}
...
export function SixCheckStep({
  ...
  async function loadAllData() {
    try {
      ...
      // Load planned actions using embedded helper
      try {
        ...
        console.error('[SixCheckStep] Error loading planned actions:', helperError);
      }
