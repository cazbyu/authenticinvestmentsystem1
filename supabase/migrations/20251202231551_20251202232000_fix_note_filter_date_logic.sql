/*
  # Fix Note Filter Date Logic in Monthly History

  1. Problem
    - The note_filter CTE was filtering notes by creation date within the month
    - This caused tasks to be excluded if notes were added after the month ended
    - Example: October task with December note wouldn't appear in October history

  2. Solution
    - Remove note creation date filter from note_filter CTE
    - Filter by task/event/etc. dates in each daily CTE instead
    - Show items in the month they occurred, regardless of when notes were added

  3. Impact
    - Historical items now appear in the correct month based on when they occurred
    - Notes can be added retroactively without affecting which month shows the item
*/
...
BEGIN
  ...
  RETURN QUERY
  WITH note_filter AS (
    -- Filter for items that have real notes (content or attachments)
    -- No date filter here - we want all notes regardless of when they were created
    SELECT DISTINCT
      j.parent_type,
      j.parent_id,
      j.note_id
...
COMMENT ON FUNCTION get_month_dates_with_items IS
  'Returns dates with item counts based on when items occurred, not when notes were created. Fixed note date filter logic.';
