/*
  # Fix Deposit Ideas Visibility in Date Range Function

  1. Changes
     - Remove the filter that hides inactive deposit ideas
     - Show ALL deposit ideas (both activated and not activated) in the reflection report
     - This allows users to see their deposit ideas in the daily view

  2. Notes
     - Previously only showing deposit ideas with is_active = true
     - Now shows all deposit ideas that aren't archived
*/
...
COMMENT ON FUNCTION get_dates_with_items_by_range IS
  'Returns all dates in range with item counts based on note creation dates. Returns all dates even if empty. Shows all deposit ideas regardless of activation status.';
