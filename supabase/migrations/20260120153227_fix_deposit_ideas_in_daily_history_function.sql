/*
  # Fix Deposit Ideas Visibility in Daily History Function

  1. Changes
     - Remove is_active filter from get_daily_history_items function
     - Show ALL deposit ideas (both activated and not activated) in Daily View
     - This fixes the "No reflections or notes recorded" message when there are inactive deposit ideas

  2. Notes
     - Inactive deposit ideas (is_active = false) should still appear in the Daily View
     - They only become "activated" when the user explicitly creates a task from them
*/
...
BEGIN
  ...
  WITH note_candidates AS (
    -- All notes that belong to this user and are "real" (have content or attachments)
    SELECT
      j.parent_type,
      j.parent_id,
      n.id AS note_id,
  ...
  ranked_notes AS (
    -- Rank notes per parent and count total notes for that parent
    SELECT
      fn.*,
      ROW_NUMBER() OVER (
  ...
  latest_notes AS (
    -- One row per parent item with the latest note
    SELECT
      parent_type,
      parent_id,
      note_id,
  ...
  deposit_idea_rows AS (
    -- Deposit ideas (show ALL, both activated and not activated)
    SELECT
...
COMMENT ON FUNCTION get_daily_history_items(date, uuid) IS
  'Returns daily reflections and note-backed items. Shows all deposit ideas regardless of activation status.';
