/*
  ...
  1. New Functions
    ...
    - `get_daily_items_summary` - Returns titles and counts for a specific date

  2. Performance
    - Uses CTEs for efficient aggregation
    - Leverages existing indexes on user_id and date columns
...
CREATE INDEX IF NOT EXISTS idx_reflections_user_created_month
  ON "0008-ap-reflections" (user_id, date_trunc('month', created_at));

CREATE INDEX IF NOT EXISTS idx_tasks_user_completed_month
  ON "0008-ap-tasks" (user_id, date_trunc('month', COALESCE(completed_at, created_at)))
  WHERE deleted_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_deposit_ideas_user_created_month
  ON "0008-ap-deposit-ideas" (user_id, date_trunc('month', created_at));

CREATE INDEX IF NOT EXISTS idx_withdrawals_user_withdrawn_month
  ON "0008-ap-withdrawals" (user_id, date_trunc('month', withdrawn_at));
