/*
  # Fix Deposit Ideas Type in Date Range Function

  1. Changes
     - Change deposit ideas type from 'note' to 'depositIdea' in the item_details
     - This allows the frontend to distinguish deposit ideas from regular task/event notes
     - Enables proper filtering in the Reflect tab

  2. Notes
     - Deposit ideas should have their own distinct type for filtering
     - Regular notes from tasks/events remain as type 'note'
*/
...
BEGIN
  ...
  daily_deposit_ideas AS (
    ...
    -- Use 'depositIdea' type to distinguish from regular notes
    SELECT
      nwd.note_date AS date_val,
...
COMMENT ON FUNCTION get_dates_with_items_by_range IS
  'Returns all dates in range with item counts based on note creation dates. Deposit ideas use type=depositIdea for proper filtering.';
