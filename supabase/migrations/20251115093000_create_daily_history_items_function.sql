...
BEGIN
  ...
  WITH note_candidates AS (
    ...
    SELECT
      CASE
        WHEN unj.parent_type = 'task' THEN COALESCE(t.type, 'task')
    ...
    LEFT JOIN "0008-ap-tasks" t
      ON unj.parent_type = 'task'
     AND t.id = unj.parent_id
    WHERE unj.user_id = v_user_id
      AND n.user_id = v_user_id
      -- Anchor notes to the user's local calendar day so all history views agree.
  ...
  qualified_notes AS (
    ...
    WHERE
      (nc.content IS NOT NULL AND btrim(nc.content) <> '')
      OR nc.has_attachment = TRUE
  ),
  ranked_notes AS (
    SELECT
      qn.parent_type,
      ...
      qn.created_at,
      ROW_NUMBER() OVER (
        PARTITION BY qn.parent_type, qn.parent_id
        ORDER BY qn.created_at DESC, qn.note_id
      ) AS note_rank
  ...
  event_rows AS (
    ...
    INNER JOIN latest_notes ln
      ON ln.parent_id = t.id
     AND ln.parent_type = 'event'
    INNER JOIN notes_per_parent np
      ON np.parent_id = t.id
     AND np.parent_type = 'event'
    WHERE t.user_id = v_user_id
      AND t.type = 'event'
      AND t.deleted_at IS NULL
  ),
