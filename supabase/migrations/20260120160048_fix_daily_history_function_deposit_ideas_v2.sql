/*
  # Re-apply Daily History Function Fix for Deposit Ideas
  
  This ensures the function shows ALL deposit ideas regardless of activation status.
  The previous migration may not have taken effect properly.
*/
...
BEGIN
  ...
  deposit_idea_rows AS (
    -- CRITICAL FIX: Show ALL deposit ideas (activated AND not activated)
    -- Removed: AND COALESCE(d.is_active, true) = true
    SELECT
    ...
    WHERE d.user_id = v_user_id
      ...
      -- NOTE: NOT filtering by is_active so inactive deposit ideas appear
  ),
  withdrawal_rows AS (
    SELECT
