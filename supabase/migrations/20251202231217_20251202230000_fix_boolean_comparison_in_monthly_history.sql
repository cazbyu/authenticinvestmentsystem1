/*
  # Fix Boolean Comparison Error in Monthly History Function

  1. Problem
    - The get_month_dates_with_items function was comparing boolean columns (daily_rose, daily_thorn) 
      with empty strings, causing "invalid input syntax for type boolean" error
    - This prevented the function from returning any data

  2. Solution
    - Change comparison from `daily_rose <> ''` to `daily_rose = true`
    - Change comparison from `daily_thorn <> ''` to `daily_thorn = true`
    - Properly handle boolean type columns

  3. Impact
    - Function will now work correctly and return monthly history data
    - Rose and thorn reflections will be properly categorized
*/
...
COMMENT ON FUNCTION get_month_dates_with_items IS
  'Returns dates with item counts and structured item details (type + title) for icon display. Fixed boolean comparison.';
