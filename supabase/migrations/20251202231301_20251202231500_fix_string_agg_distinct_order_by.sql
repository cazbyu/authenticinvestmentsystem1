/*
  # Fix STRING_AGG DISTINCT ORDER BY Error

  1. Problem
    - PostgreSQL doesn't allow ORDER BY in STRING_AGG when using DISTINCT
    - Error: "in an aggregate with DISTINCT, ORDER BY expressions must appear in argument list"

  2. Solution
    - Remove DISTINCT from STRING_AGG calls since we're already grouping by date
    - Keep ORDER BY for proper sorting
    - The grouping ensures we don't get duplicates anyway

  3. Impact
    - Function will now work correctly
    - Content summaries will be properly sorted
*/
...
COMMENT ON FUNCTION get_month_dates_with_items IS
  'Returns dates with item counts and structured item details (type + title) for icon display. Fixed boolean and STRING_AGG issues.';
